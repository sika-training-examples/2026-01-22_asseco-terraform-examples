resource "azurerm_network_interface" "vm2" {
  resource_group_name = azurerm_resource_group.training.name
  location            = azurerm_resource_group.training.location

  name = "${azurerm_resource_group.training.name}-vm2-nic"

  ip_configuration {
    name                          = "internal"
    subnet_id                     = azurerm_subnet.net2-default.id
    private_ip_address_allocation = "Static"
    private_ip_address            = "10.20.0.102"
    public_ip_address_id          = azurerm_public_ip.ip-2.id
  }
}

resource "azurerm_network_interface_security_group_association" "vm2-allow22" {
  network_interface_id      = azurerm_network_interface.vm2.id
  network_security_group_id = azurerm_network_security_group.allow22.id
}

resource "azurerm_linux_virtual_machine" "vm2" {
  resource_group_name = azurerm_resource_group.training.name
  location            = azurerm_resource_group.training.location

  name = "${azurerm_resource_group.training.name}-vm2"
  size = "Standard_B1ls" // 1 CPU, 0.5 RAM https://cloudprice.net/vm/Standard_B1ls

  network_interface_ids = [
    azurerm_network_interface.vm2.id,
  ]

  admin_username = "azadmin"
  admin_ssh_key {
    username   = "azadmin"
    public_key = "ssh-rsa  root@lab0"
  }

  os_disk {
    caching              = "ReadWrite"
    storage_account_type = "Standard_LRS"
  }

  source_image_reference {
    publisher = "Debian"
    offer     = "debian-13"
    sku       = "13"
    version   = "latest"
  }
}

output "ssh_vm2" {
  value = "ssh azadmin@${azurerm_public_ip.ip-2.ip_address}"
}
